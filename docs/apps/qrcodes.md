...
`templates/qrcodes/label.html` (etiqueta antiga), `label_square.html` (etiqueta nova) e `qr_grid.html` (grade A4 de QR puro, 16/09/2026) — renderizados só internamente via `render_to_string` → WeasyPrint, nunca servidos como página normal. `qr_grid.html` recebe `pages` (lista de páginas já paginadas em Python, cada uma com até `QR_GRID_PAGE_SIZE` células), `page_width_mm`/`page_height_mm`/`cell_size_mm`/`gutter_mm`/`margin_horizontal_mm`/`margin_vertical_mm` (nenhuma medida fixa no template, `@page{margin:0}` e toda margem visível vem do `padding` calculado do `.qr-page`) e `theme` (`<body class="theme-dark">` quando "dark" — mesmo padrão de `label_square.html`: só `body.theme-dark`/`.qr-page`/`.qr-caption` ganham regra de cor extra, o `.qr-image` nunca muda). Cada célula tem um campo `caption` (sempre `None` hoje) como ponto de extensão futuro, sem nenhuma opção de UI para preenchê-lo ainda. **Bug corrigido no mesmo dia (dimensionamento)**: `{{ margin_vertical_mm }}` (13.5, fracionário) renderizava com VÍRGULA decimal por causa de `LANGUAGE_CODE="pt-br"` ("13,5mm", CSS inválido, WeasyPrint ignorava e colapsava a margem para 0 — a causa raiz do "grade deslocada para a esquerda" relatado) — corrigido com `{% load l10n %}{% localize off %}` ao redor do `<style>`.
