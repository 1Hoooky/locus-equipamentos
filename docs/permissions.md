...
## FECHAMENTO DA PROPOSTA COMERCIAL (23/09/2026) — nenhuma `Permission` nova

`ProposalInstallmentAddView`/`UpdateView`/`RemoveView` (adicionar/editar/remover parcela de pagamento) reaproveitam `("crm.view_opportunities", "crm.change_opportunities")` — a MESMA dupla já usada por `ProposalItemAddView`/`UpdateView`/`RemoveView`/`ProposalConditionsSaveView`. O raciocínio é idêntico ao já registrado para item/condições comerciais (ver `docs/apps/crm.md`, seção Permissions): configurar parcela é conceitualmente "editar dados da proposta em rascunho", não uma ação qualitativamente nova como emitir (`issue_proposal_documents`) ou gerar contrato (`generate_contract`) — criar uma permissão dedicada só para parcela duplicaria uma distinção que já existe (rascunho vs. documento formal emitido). O catálogo `crm.*` continua com **11** entradas — nenhuma soma nesta rodada. A regra de "soma das parcelas = total" que BLOQUEIA a emissão (`validate_payment_before_issue()`) é uma validação de negócio dentro do service, não uma checagem de autorização — não tem relação com `Permission`/`Cargo`.
