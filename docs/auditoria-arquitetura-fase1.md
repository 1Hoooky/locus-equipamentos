...
| Reemitir patrimônio (fluxo excepcional) | Admin | `CAN_SUPERSEDE_EQUIPMENT` | Sim | `EquipmentSupersedeViewTest` |
| ~~Editar `code` de modelo com equipamentos~~ | — | ~~`CAN_EDIT_LOCKED_MODEL_CODE`~~ (removida em 25/08/2026) | **Correção:** não existe esse fluxo. `code` é travado incondicionalmente para TODOS os perfis (inclusive Admin/superusuário) assim que o modelo tem equipamento vinculado — em `EquipmentModelForm`, no Django admin e, na camada que garante a regra de fato, `EquipmentModel.clean()`. A constante sugeria um caminho excepcional que nunca foi implementado; removida em vez de inventado um bypass só para "usá-la" (ver `apps/accounts/permissions.py`) | N/A — não há comportamento a testar |
...
| Ver valor de aquisição / dados financeiros | Admin, Administrativo | `CAN_VIEW_ACQUISITION_VALUE` | **Corrigido em 25/08/2026:** aplicada agora em `EquipmentDetailView` (`apps/equipment/views.py`), não só no template — para quem não tem a permissão, a consulta ao banco já usa `.defer("supplier", "acquisition_date", "acquisition_value")`, então o dado nem é carregado no objeto, além do template continuar escondendo o bloco (`{% if can_view_acquisition_value %}`, agora vindo da view em vez de `user.is_administrativo_ou_superior`) | Sim — `AcquisitionValueVisibilityByRoleTest` (`test_public_detail_view.py`), inclusive checando `equipment.get_deferred_fields()` |
...
| Adicionar fotos | Admin, Administrativo, Operacional | `CAN_ADD_PHOTOS` | Constante definida, sem uso — `apps/attachments` ainda é esqueleto (ver seção 5). Comentário inline em `permissions.py` marcando explicitamente o escopo (Fase 2/3), revisado em 25/08/2026 | N/A |
| ~~Consultar equipamento e histórico~~ | Todos os perfis | ~~`CAN_VIEW_EQUIPMENT`~~ (removida em 25/08/2026) | **Correção:** a constante nunca foi referenciada em nenhuma view — `EquipmentListView` e o ramo autenticado de `EquipmentDetailView` já usam só `request.user.is_authenticated`, o que hoje equivale a "todos os 4 perfis" (não existe perfil autenticado sem acesso de consulta). Sem distinção real de papel para esta ação, a constante não tinha propósito — removida em vez de forçada a ser usada só para deixar de estar "morta" | `test_public_detail_view.py`, `test_permissions.py` (continuam válidos — testam o comportamento real de `is_authenticated`) |
...
2. ~~**Visibilidade de valor de aquisição sem teste automatizado.**~~ **Resolvido em 25/08/2026** — ver seção 8 (Correções de 25/08/2026, item 1).
...
## 8. Correções de 25/08/2026 (auditoria final da Fase 1 — fechamento de inconsistências)

Depois do congelamento acima, uma segunda rodada de auditoria (focada em comportamento real contra PostgreSQL, não em arquitetura) encontrou e corrigiu 2 bugs objetivos sem relação com este documento (link do cabeçalho para usuário anônimo; crash de filtro `category`/`model` não numérico — ver relatório da auditoria final entregue ao usuário) e sinalizou os achados de arquitetura abaixo, que o usuário pediu para fechar antes de abrir a Fase 2:

1. **`CAN_VIEW_ACQUISITION_VALUE` agora é aplicada no backend, não só no template.** `EquipmentDetailView` (`apps/equipment/views.py`) calcula `can_view_acquisition_value` a partir da constante centralizada (mesmo padrão de `RoleRequiredMixin`: `is_superuser` ou `role` na lista) e, quando o usuário não tem a permissão, usa `.defer("supplier", "acquisition_date", "acquisition_value")` na própria queryset — o dado não é só escondido na tela, ele não é sequer trazido do banco para o objeto `equipment` nesse caso. `detail_private.html` passou a usar `can_view_acquisition_value` (vindo da view) em vez de `user.is_administrativo_ou_superior` (propriedade do model de usuário, que não necessariamente reflete a lista de perfis da permissão centralizada nem cobre superusuário com `role` diferente). Testado em `AcquisitionValueVisibilityByRoleTest` (`apps/equipment/tests/test_public_detail_view.py`), incluindo asserção direta sobre `equipment.get_deferred_fields()` para os 4 perfis, superusuário com role sem a permissão, e a rota pública.
2. **`CAN_EDIT_LOCKED_MODEL_CODE` removida.** Auditada a intenção original: a constante sugeria um fluxo excepcional de Admin para editar `EquipmentModel.code` após haver equipamento vinculado. Esse fluxo nunca existiu — a trava (`EquipmentModelForm`, Django admin, e `EquipmentModel.clean()`) é incondicional para todos os perfis, sem exceção nem para superusuário. Como não existe hoje um procedimento seguro e auditado para essa correção excepcional, e criar um agora estaria fora do pedido, a constante foi removida em vez de mantida morta ou "usada" artificialmente. A correção de um `code` errado depois desse ponto continua sendo, por design, um procedimento fora do CRUD comum (edição direta no banco, sob responsabilidade de quem administra a infraestrutura).
3. **`CAN_VIEW_EQUIPMENT` removida** — nunca foi referenciada em nenhuma view; o comportamento real (todos os 4 perfis autenticados podem consultar) já era garantido só por `is_authenticated`/`LoginRequiredMixin`. Sem uso genuíno para uma constante de papel aqui hoje; documentado em `permissions.py` o motivo e a condição em que ela voltaria a fazer sentido (um 5º perfil sem acesso de consulta).
4. **`CAN_ADD_PHOTOS` mantida**, com comentário inline explícito marcando o escopo (Fase 2/3, fotos/anexos de equipamento — `apps/attachments` continua esqueleto vazio). Mesmo tratamento já dado a `CAN_REGISTER_OPERATIONS`: constante correta, intencionalmente sem uso ainda, documentada como tal — não é dead code por descuido.

Nenhuma mudança de comportamento visível para os perfis já autorizados (Admin/Administrativo continuam vendo os mesmos dados, exatamente como antes); a única mudança de comportamento observável é que a proteção de Operacional/Consulta passou a ser garantida em duas camadas (view + template) em vez de uma só, com prova automatizada em nível de banco/contexto, não só de HTML renderizado.
